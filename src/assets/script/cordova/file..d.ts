 ...
 *
 * @param type              Whether the filesystem requested should be persistent,
 *                          as defined above. Use one of TEMPORARY or PERSISTENT.
 * @param size              This is an indicator of how much storage space, in bytes,
 *                          the application expects to need.
 * @param successCallback   The callback that is called when the user agent provides a filesystem.
 * @param errorCallback     A callback that is called when errors happen,
 *                          or when the request to obtain the filesystem is denied.
 */
declare function requestFileSystem(type: LocalFileSystem, size: number,
 ...
 */
declare function resolveLocalFileSystemURL(url: string,
 ...
 * @param errorCallback    invoked if error occurs retrieving file system entry
 */
declare function resolveLocalFileSystemURI(uri: string,
...
interface Entry {
   ...
   * A move of a directory on top of an existing empty directory
   * must attempt to delete and replace that directory.
   *
   ...
   * @param errorCallback   A callback that is called when errors happen.
   */
  moveTo: (parent: DirectoryEntry, newName?: string,
    successCallback?: (entry: Entry) => void,
    errorCallback?: (error: FileError) => void) => void;
  /**
   ...
   *     A copy of a directory on top of an existing empty directory
   *     must attempt to delete and replace that directory.
   ...
   * @param errorCallback A callback that is called when errors happen.
   */
  copyTo: (parent: DirectoryEntry, newName?: string,
    successCallback?: (entry: Entry) => void,
    errorCallback?: (error: FileError) => void) => void;
  /**
   ...
   * that is not empty. It is an error to attempt to delete the root directory of a filesystem.
   *
...
interface DirectoryEntry extends Entry {
   ...
   *                It is an error to attempt to create a file
   *                whose immediate parent does not yet exist.
   * @param options If create and exclusive are both true, and the path already exists,
   *                getFile must fail.
   *                If create is true, the path doesn't exist, and no other error occurs,
   *                getFile must create it as a zero-length file and
   *                return a corresponding FileEntry.
   *                If create is not true and the path doesn't exist, getFile must fail.
   *                If create is not true and the path exists, but is a directory,
   *                getFile must fail.
   *                Otherwise, if no other error occurs, getFile
   *                must return a FileEntry corresponding to path.
   ...
   *                It is an error to attempt to create a directory
   *                whose immediate parent does not yet exist.
   * @param options If create and exclusive are both true and the path already exists,
   *                getDirectory must fail.
   *                If create is true, the path doesn't exist, and no other error occurs,
   *                getDirectory must create and return a corresponding DirectoryEntry.
   *                If create is not true and the path doesn't exist, getDirectory must fail.
   *                If create is not true and the path exists, but is a file,
   *                getDirectory must fail.
   *                Otherwise, if no other error occurs,
   *                getDirectory must return a DirectoryEntry corresponding to path.
   ...
   * of the directory may be deleted.
...
interface Flags {
  ...
  create?: boolean;
  /**
   * By itself, exclusive must have no effect. Used with create,
   * it must cause getFile and getDirectory to fail if the target path already exists.
   */
  exclusive?: boolean;
}
 ...
 *     Once all entries have been returned,
 *     the next call to readEntries must produce an empty array.
 *     If not all entries have been returned, the array produced by readEntries must not be empty.
 *     The entries produced by readEntries must not
 *     include the directory itself ["."] or its parent [".."].
 */
interface DirectoryReader {
   ...
   *                        of readEntries, successCallback must be called
   *                        with a zero-length array as an argument.
...
interface FileWriter extends FileSaver {
   ...
   * This always less or equal than length.
   ...
   * If shortening the file, data beyond the new length
   * will be discarded. If extending the file,
   * the existing data will be zero-padded up to the new length.
   *
