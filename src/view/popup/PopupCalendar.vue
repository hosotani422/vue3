...
<PopupModal class="popupCalendar" :open="calendar.state.open"
  ...
  <PartLayout class="flex auto column gap-2l">
    ...
    <PartLayout tag="ul" class="flex">
      <PartText tag="li" class="even center font-s" :key="`week${week}`"
        v-for="week of calendar.getter.labelWeek.value()">{{week}}</PartText>
    </PartLayout>
  </PartLayout>
  <PartLayout class="body auto hiddenXY">
    <PartLayout class="area flex" @touchstart="calendar.action.swipeInit({event:$event})">
      <PartLayout tag="ul" class="month even flex wrap"
        :key="`month${month.id}`" v-for="month of calendar.getter.labelDay.value()">
        <PartLayout tag="li"
          class="day flex align-center justify-center padding-m font-m"
          :class="{select:day.select,today:day.today,hide:day.hide}"
          :key="`day${month.id}${day.id}`" v-for="day of month.day"
          @click="calendar.state.callback(day.id)">{{day.text}}</PartLayout>
      </PartLayout>
    </PartLayout>
  </PartLayout>
  <PartLayout class="auto flex justify-end gap-2l">
    ...
    <InputButton class="auto error" @click="calendar.state.callback(``)">{{calendar.state.clear}}</InputButton>
  </PartLayout>
</PopupModal>
</template>
